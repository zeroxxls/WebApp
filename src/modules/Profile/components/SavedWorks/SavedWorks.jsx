...
import { useSavedWorks } from '../../hooks/save/useSaveWorks';
import { SavedWorksHeader } from '../SavedWorks/SavedWorksHeader';
import { NoSavedWorks } from '../Errors/NoSavedWorks';
...
export const SavedWorks = () => {
  ...
  const { savedWorks, loading } = useSavedWorks(userId);
  ...
  return (
    <div className="flex flex-col h-full">
      <SavedWorksHeader/>
      {savedWorks.length > 0 ? (
        <SavedWorksGrid works={savedWorks} onWorkClick={openWorkModal} />
      ) : (
        <NoSavedWorks />
      )}
